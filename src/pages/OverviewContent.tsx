...
export default function Overview() {
  ...
  return (
    <>
      ...
      {/* Splash screen */}
      {showSplash && (
        <div className={`fixed inset-0 z-[100] flex flex-col items-center justify-center bg-[#09090b] transition-opacity duration-500 ease-out ${fadeSplash ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
          <div className="relative flex flex-col items-center gap-6">
            <div className="relative h-64 w-64 md:h-80 md:w-80 overflow-hidden rounded-2xl border border-white/5 shadow-2xl flex items-center justify-center bg-black/25">
              <img src="/atlas.gif" alt="Atlas Loader" className="h-full w-full object-cover select-none pointer-events-none" />
            </div>
            <div className="flex flex-col items-center gap-2">
              <h2 className="text-xl font-bold tracking-wider text-foreground uppercase animate-pulse">Yourspeak Monitoring Hub</h2>
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <span className="h-1.5 w-1.5 rounded-full bg-primary animate-ping" />
                <span>Initializing Unified Devops Control...</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-border bg-card/40 backdrop-blur-md p-5 flex flex-wrap items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">Unified operations center displaying real-time repository pipeline status and host cluster health.</p>
          </div>
          {/* Connection status pills */}
          <div className="flex items-center gap-3 text-xs font-mono">
            <div className="flex items-center gap-1.5 bg-zinc-950 px-3 py-1.5 rounded-lg border border-zinc-800">
              <span className={cn("h-1.5 w-1.5 rounded-full", hasToken ? "bg-emerald-500" : "bg-rose-500")} />
              <span className="text-muted-foreground text-[10px]">GitHub: {hasToken ? "ONLINE" : "OFFLINE"}</span>
            </div>
            <div className="flex items-center gap-1.5 bg-zinc-950 px-3 py-1.5 rounded-lg border border-zinc-800">
              <span className={cn("h-1.5 w-1.5 rounded-full", k8sClusterQuery.isSuccess ? "bg-emerald-500" : "bg-rose-500")} />
              <span className="text-muted-foreground text-[10px]">SSH VM: {k8sClusterQuery.isSuccess ? "CONNECTED" : "DISCONNECTED"}</span>
            </div>
          </div>
        </div>
        ...
        {!hasToken && (
          <div className="flex items-center gap-3 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3">
            ...
            <p className="text-xs text-amber-300">GitHub Token not configured. Please go to <strong>Settings</strong> to set your GitHub PAT to see repository metrics.</p>
          </div>
        )}
              ...
              <div className="lg:col-span-7">
                <div className="rounded-xl border border-border bg-card p-5 space-y-4 h-full flex flex-col">
                  ...
                  {k8sClusterQuery.isLoading ? (
                    <div className="space-y-4 animate-pulse py-2 flex-1">
        ...
        {/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
            DOCKER OVERVIEW GROUP
        â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
        <DockerOverviewSection environment={environment} />

        {/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
            GITHUB OVERVIEW GROUP
        â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
        <div className="rounded-xl border border-blue-500/20 bg-card/40 backdrop-blur-sm overflow-hidden">
          {/* Group header */}
          <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-blue-500/15 bg-blue-500/5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-blue-300 uppercase tracking-widest">GitHub Overview</span>
              <span className={cn("h-1.5 w-1.5 rounded-full ml-1", hasToken ? "bg-emerald-500" : "bg-rose-500")} />
              <span className="text-[10px] text-zinc-500 font-mono">{hasToken ? "ONLINE" : "OFFLINE"}</span>
            </div>
            {primaryRepo ? (
              <div className="flex items-center gap-1.5 bg-zinc-950 px-3 py-1.5 rounded-lg border border-zinc-800">
                <GitBranch className="h-3.5 w-3.5 text-blue-400 flex-shrink-0" />
                <select
                  className="bg-transparent border-none text-xs font-mono text-foreground focus:outline-none cursor-pointer max-w-[200px] truncate"
                  value={primaryRepo.id}
                  onChange={(e) => setSelectedRepoId(Number(e.target.value))}
                >
                  {repos.map(repo => (
                    <option key={repo.id} value={repo.id} className="bg-zinc-950 text-foreground">
              ...
              <span className="text-[10px] text-zinc-600 font-mono italic">No repository selected</span>
            )}
          </div>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="grid gap-4 grid-cols-1 sm:grid-cols-3">
              <StatCard
                icon={Users} label="Repository Authors" color="text-blue-400" bg="bg-blue-500/10"
                loading={commitsQuery.isLoading}
                value={todayAuthors.length || commits.length > 0 ? todayAuthors.length : "-"}
                sub={commits.length > 0 ? `${commits.length} commits synced` : "No commits"}
                onClick={openAuthorsPanel}
              />
              <StatCard
                icon={GitPullRequest} label="Open PRs" color="text-purple-400" bg="bg-purple-500/10"
                loading={prsQuery.isLoading}
                value={openPRs.length > 0 ? openPRs.length : prs.length > 0 ? 0 : "-"}
                sub={prs.length > 0 ? `${prs.length} total (PR)` : undefined}
                onClick={openPRsPanel}
              />
              <StatCard
                icon={CheckCircle2} label="CI/CD Pipelines" color="text-emerald-400" bg="bg-emerald-500/10"
                loading={runsQuery.isLoading}
                value={runStats.total > 0 ? `${runStats.success}/${runStats.total}` : "-"}
                sub={runs[0] ? `Recent: ${runs[0].conclusion || runs[0].status}` : undefined}
                onClick={openCICDPanel}
              />
            </div>
              ...
              <div className="lg:col-span-7 space-y-4">
                ...
                <div className="block w-full text-left rounded-xl border border-border bg-card p-5 space-y-4 hover:border-emerald-500/50 transition-colors group">
                  <div className="flex items-center justify-between border-b border-border pb-3">
                    <div>
                      <h2 className="font-semibold text-sm group-hover:text-emerald-400 transition-colors">Commit Velocity</h2>
                      <p className="text-xs text-muted-foreground mt-0.5">Click a data point to see specific commits</p>
                    </div>
                    <Activity className="h-4 w-4 text-emerald-400" />
                  </div>
                  {commitsQuery.isLoading ? (
                    <div className="h-[140px] rounded-lg bg-muted/20 animate-pulse" />
                  ) : commits.length === 0 ? (
                    <div className="h-[140px] flex items-center justify-center text-xs text-muted-foreground">No commits sync logs.</div>
                  ) : (
                    <ResponsiveContainer width="100%" height={140}>
                      <AreaChart
                        data={commitSparkline}
                        margin={{ top: 10, right: 10, bottom: 0, left: -25 }}
                        onClick={(e) => {
                          if (e && e.activePayload && e.activePayload.length > 0) {
                            openCommitsPanelForDate(e.activePayload[0].payload.fullDate)
                          }
                        }}
                      ...
                      >
                        ...
                        <XAxis dataKey="date" tick={{ fontSize: 9, fill: "#71717a" }} tickLine={false} axisLine={false} interval={2} />
                        <YAxis tick={{ fontSize: 9, fill: "#71717a" }} tickLine={false} axisLine={false} />
                        <Tooltip
                          content={({ active, payload }) => active && payload?.length ? (
                            <div className="rounded-lg border border-border bg-card px-2.5 py-1 text-[11px] shadow-lg">
                              <span className="font-bold text-emerald-400">{payload[0].value} commits</span>
                              <span className="text-muted-foreground ml-1">on {payload[0].payload.date}</span>
                            </div>
                          ) : null}
                        />
                        <Area type="monotone" dataKey="count" stroke="#10b981" strokeWidth={2} fill="url(#commitGrad)" dot={false} />
                      </AreaChart>
                    </ResponsiveContainer>
                  )}
                </div>

                {/* PR Velocity Sparkline Chart */}
                <div className="block w-full text-left rounded-xl border border-border bg-card p-5 space-y-4 hover:border-purple-500/50 transition-colors group">
                  <div className="flex items-center justify-between border-b border-border pb-3">
                    <div>
                      <h2 className="font-semibold text-sm group-hover:text-purple-400 transition-colors">Pull Request Velocity</h2>
                      <p className="text-xs text-muted-foreground mt-0.5">Click a data point to see specific pull requests</p>
                    </div>
                  ...
                  {prsQuery.isLoading ? (
                    ...
                    <div className="h-[140px] flex items-center justify-center text-xs text-muted-foreground">No pull requests found.</div>
                  ) : (
                    <ResponsiveContainer width="100%" height={140}>
                      <AreaChart 
                        data={prSparkline} 
                        ...
                        onClick={(e) => {
                          if (e && e.activePayload && e.activePayload.length > 0) {
                            openPRsPanelForDate(e.activePayload[0].payload.fullDate)
                          }
                        }}
                        style={{ cursor: 'pointer' }}
                      >
                        ...
                        <Tooltip
                          content={({ active, payload }) => active && payload?.length ? (
                            <div className="rounded-lg border border-border bg-card px-2.5 py-1 text-[11px] shadow-lg">
                              <span className="font-bold text-purple-400">{payload[0].value} PRs</span>
                              <span className="text-muted-foreground ml-1">on {payload[0].payload.date}</span>
                            </div>
                          ) : null}
                        />
                        <Area type="monotone" dataKey="count" stroke="#c084fc" strokeWidth={2} fill="url(#prGrad)" dot={false} />
                      </AreaChart>
                    </ResponsiveContainer>
                  )}
                </div>
                ...
                <div className="rounded-xl border border-border bg-card overflow-hidden">
                  <div className="px-5 py-4 border-b border-border flex items-center justify-between">
                    <div>
                      <h2 className="font-semibold text-sm">CI/CD Run Pipelines</h2>
                      <p className="text-xs text-muted-foreground mt-0.5">Recent automated build execution logs</p>
                    </div>
                  ...
                  <div className="divide-y divide-border">
                    {runsQuery.isLoading ? (
                      ...
                      <div className="px-5 py-8 text-center text-xs text-muted-foreground">No workflow runs found.</div>
                    ) : (
                      recentRuns.map((run: any) => (
                        <a key={run.id} href={run.html_url} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-3 px-5 py-3 hover:bg-accent/15 transition-colors group">
                          <div className="flex-shrink-0">{statusIcon(run.conclusion, run.status)}</div>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-semibold text-zinc-200 truncate group-hover:text-primary transition-colors">{run.name || run.display_title}</p>
                            <p className="text-[10px] text-muted-foreground font-mono truncate mt-0.5">
                              #{run.run_number} · branch: {run.head_branch}
                            </p>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <span className={cn("text-[9px] font-bold px-2 py-0.5 rounded-full border uppercase tracking-wider", conclusionColor(run.conclusion, run.status))}>
                              {run.status === "in_progress" || run.status === "queued" ? "running" : run.conclusion || run.status}
                            </span>
                ...
                <div className="rounded-xl border border-border bg-card overflow-hidden">
                  <div className="px-5 py-4 border-b border-border flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">Ranked by commits in the current branch</p>
                    </div>
                    ...
                    {contributorsQuery.isLoading ? (
                      Array.from({ length: 3 }).map((_, i) => (
                        <div key={i} className="flex items-center gap-3 px-5 py-3 animate-pulse">
                          <div className="h-6 w-6 rounded-full bg-muted/50 flex-shrink-0" />
                      ...
                      <div className="px-5 py-8 text-center text-xs text-muted-foreground">No contributors data found.</div>
                    ) : (
                      topContributors.map((c: any, i) => (
                        <a key={c.login} href={`https://github.com/${c.login}`} target="_blank" rel="noopener noreferrer"
                          ...
                          {c.avatar_url ? (
                            <img src={c.avatar_url} alt={c.login} className="h-6 w-6 rounded-full flex-shrink-0 border border-zinc-800" />
                          ) : (
                            <div className="h-6 w-6 rounded-full flex-shrink-0 bg-primary/10 flex items-center justify-center text-[10px] font-bold text-primary">
                              {c.login?.[0]?.toUpperCase()}
                            </div>
                          )}
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-semibold text-zinc-300 group-hover:text-primary transition-colors">{c.login}</p>
                            <p className="text-[10px] text-muted-foreground mt-0.5">{c.contributions} commits</p>
                          </div>
                          <div className="flex-shrink-0">
                            <div className="h-1 rounded-full bg-zinc-800 overflow-hidden w-16">
                              <div
                                className="h-full bg-primary"
                                style={{ width: `${Math.min(100, (c.contributions / (topContributors[0]?.contributions || 1)) * 100)}%` }}
                              />
                            </div>
                          </div>
                        </a>
